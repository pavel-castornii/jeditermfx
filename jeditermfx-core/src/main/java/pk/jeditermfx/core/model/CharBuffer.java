...
public class CharBuffer implements Iterable<Character>, CharSequence {
    ...
    public CharBuffer(@NotNull char[] buf, int start, int length) {
        ...
        if (myStart + myLength > myBuf.length) {
            throw new IllegalStateException(String.format("Interval is out of array bounds: %d+%d>%d",
                    myStart, myLength, myBuf.length));
        }
    }
    ...
    public CharBuffer subBuffer(Pair<Integer, Integer> range) {
        return new CharBuffer(myBuf, getStart() + range.getFirst(), range.getSecond() - range.getFirst());
    }
